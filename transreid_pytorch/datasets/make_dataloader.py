...
__factory = {
    ...
    'msmt17': MSMT17,
    'mm': MM,
    'syn_dark_market_msmt': SynDarkMarketMSMT,
...
def make_dataloader(cfg, is_train=True):
    ...
    if is_train and cfg.DISTILL.ENABLED:
        ...
        aux_loader = DataLoader(
            ...
            sampler=RandomIdentitySampler(aux_sampler_source, cfg.SOLVER.IMS_PER_BATCH, cfg.DATALOADER.NUM_INSTANCE),
            num_workers=num_workers,
            collate_fn=srd_pair_collate_fn,
        )
        print('using SRD paired auxiliary dataset: {} images, {} ids'.format(len(aux_pairs), aux_num_classes))

    return train_loader, train_loader_normal, val_loader, len(dataset.query), num_classes, cam_num, view_num, aux_loader, aux_num_classes
