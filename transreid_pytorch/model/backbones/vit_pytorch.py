...
from ..modules.ng_tse import NormGuidedTokenStructureEnhancement
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        if ngtse_attn_cfg is not None:
            ...
            gate_temp = max(float(ngtse_attn_cfg.get('attention_gate_temp', 0.1)), eps)
            q_cls_norm = q.norm(dim=-1)[:, :, 0:1]
            k_patch_norm = k.norm(dim=-1)[:, :, 1:]
            if ngtse_attn_cfg.get('detach_gate', True):
                q_cls_norm = q_cls_norm.detach()
                k_patch_norm = k_patch_norm.detach()
            gate = torch.sigmoid((k_patch_norm - q_cls_norm) / gate_temp)
            cls_patch_penalty = beta * gate.unsqueeze(-2) * (
                k_patch_norm.unsqueeze(-2) / q_cls_norm.unsqueeze(-1).clamp_min(eps)
            )
            attn[:, :, 0:1, 1:] = attn[:, :, 0:1, 1:] - cls_patch_penalty
        ...
        x = self.proj_drop(x)
        if return_qk_norm:
            q_norm = q.norm(dim=-1).mean(dim=1)
            k_norm = k.norm(dim=-1).mean(dim=1)
            return x, q_norm, k_norm
        if return_qnorm:
            q_norm = q.norm(dim=-1).mean(dim=1)
            return x, q_norm
        return x
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_extra_drop_path=False,
                 use_ngtse=False, ngtse_cfg=None, grid_size=None):
        super().__init__()
        ...
        self.use_extra_drop_path = use_extra_drop_path
        self.use_ngtse = use_ngtse
        self.ngtse_mode = 'token'
        self.ngtse_cfg = None
        self.ngtse_gate_type = 'none'
        if self.use_ngtse:
            if ngtse_cfg is None or grid_size is None:
                raise RuntimeError('NG-TSE requires ngtse_cfg and grid_size.')
            self.ngtse_mode = ngtse_cfg.get('mode', 'token')
            if self.ngtse_mode not in ('token', 'attention', 'both'):
                raise ValueError("NG-TSE mode must be one of 'token', 'attention', or 'both'.")
            self.ngtse_cfg = ngtse_cfg
            self.ngtse_gate_type = ngtse_cfg.get('gate_type', 'q')
            if self.ngtse_mode in ('token', 'both'):
                self.ngtse = NormGuidedTokenStructureEnhancement(
                    dim=dim,
                    grid_size=grid_size,
                    kernel_size=ngtse_cfg.get('kernel_size', 5),
                    beta=ngtse_cfg.get('beta', 0.25),
                    gate_type=self.ngtse_gate_type,
                    detach_gate=ngtse_cfg.get('detach_gate', True),
                    residual_gate=ngtse_cfg.get('residual_gate', True),
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., camera=0, view=0,drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), local_feature=False, sie_xishu =1.0, hw_ratio=1, gem_pool = False, stem_conv=False, extra_drop_path=False, extra_drop_path_layers=(11,), ngtse=False, ngtse_mode='token', ngtse_layers=(11,), ngtse_kernel=5, ngtse_beta=0.25, ngtse_gate_type='q', ngtse_detach_gate=True, ngtse_residual_gate=True, ngtse_weighted_structure=True, ngtse_attention_gate_temp=0.1):
        ...
        extra_drop_path_layers = set(int(i) for i in extra_drop_path_layers)
        ngtse_layers = set(int(i) for i in ngtse_layers)
        ngtse_cfg = dict(
            mode=ngtse_mode,
            kernel_size=ngtse_kernel,
            beta=ngtse_beta,
            gate_type=ngtse_gate_type,
            detach_gate=ngtse_detach_gate,
            residual_gate=ngtse_residual_gate,
            weighted_structure=ngtse_weighted_structure,
            attention_gate_temp=ngtse_attention_gate_temp,
        )
        grid_size = (self.patch_embed.num_y, self.patch_embed.num_x)

        self.blocks = nn.ModuleList([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                use_extra_drop_path=extra_drop_path and i in extra_drop_path_layers,
                use_ngtse=ngtse and i in ngtse_layers,
                ngtse_cfg=ngtse_cfg,
                grid_size=grid_size)
    ...
    def load_param(self, model_path,hw_ratio):
        ...
        for k, v in param_dict.items():
            ...
            try:
                ...
                print('===========================ERROR=========================')
                print('shape do not match in k :{}: param_dict{} vs self.state_dict(){}'.format(k, v.shape, model_state[k].shape))
